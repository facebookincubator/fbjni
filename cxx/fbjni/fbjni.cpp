...
std::u16string JString::toU16String() const {
  ...
  return std::u16string(
      reinterpret_cast<const char16_t*>(utf16String.chars()),
      utf16String.length());
}
...
local_ref<JString> make_jstring(const char* utf8) {
  ...
  size_t len;
  size_t modlen =
      detail::modifiedLength(reinterpret_cast<const uint8_t*>(utf8), &len);
  jstring result;
  if (modlen == len) {
    // The only difference between utf8 and modifiedUTF8 is in encoding 4-byte
    // UTF8 chars and '\0' that is encoded on 2 bytes.
    //
    // Since modifiedUTF8-encoded string can be no shorter than it's UTF8
    // conterpart we know that if those two strings are of the same length we
    // don't need to do any conversion -> no 4-byte chars nor '\0'.
    ...
    detail::utf8ToModifiedUTF8(
        ...
        len,
        reinterpret_cast<uint8_t*>(modified.data()),
        modified.size());
