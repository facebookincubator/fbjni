...
/** @file CoreClasses.h
 *
 * In CoreClasses.h wrappers for the core classes (jobject, jclass, and jstring)
 * is defined to provide access to corresponding JNI functions + some
 * conveniance.
 */
...
/// Check to see if two references refer to the same object. Comparison with
/// nullptr returns true if and only if compared to another nullptr. A weak
...
// underlying JClass, those calls can only be checked at runtime. It is
// recommended that the JavaClass-type instead explicitly expose it's methods as
...
class JObject : detail::JObjectBase {
 public:
  ...
  static constexpr detail::SimpleFixedString<0>
  get_instantiated_java_descriptor() {
    return "";
  }
  static constexpr detail::SimpleFixedString<0> get_instantiated_base_name() {
    return "";
  }
...
struct JTypeFor {
  static_assert(
      std::is_base_of<
          ...
          typename std::remove_pointer<JType>::type>::value,
      "");
...
class JavaClass : public Base {
 ...
 protected:
  ...
  /// While a JavaClass-type can expose this function directly, it is
  /// recommended to instead to use this to explicitly only expose those
  /// constructors that the Java class actually has (i.e. with static create()
  /// functions).
...
class JClass : public JavaClass<JClass, JObject, jclass> {
 public:
  ...
  /// Look up the method with given name and descriptor as specified with the
  /// type arguments
  template <typename F>
  ...
  template <typename T>
  JField<PrimitiveOrJniType<T>> getField(
      const char* name,
  ...
  local_ref<R> newObject(JConstructor<R(Args...)> constructor, Args... args)
      const;

  /// Look up the static method with given name and descriptor as specified with
  /// the type arguments
  template <typename F>
  ...
  template <typename F>
  JStaticMethod<F> getStaticMethod(const char* name, const char* descriptor)
      const;

  /// Look up the non virtual method with given name and descriptor as specified
  ...
  template <typename F>
  JNonvirtualMethod<F> getNonvirtualMethod(
      const char* name,
      const char* descriptor) const;
...
/// Convenience functions to convert a const char*, std::string, or
/// std::u16string into a @ref local_ref to a jstring.
...
class JArrayClass : public JavaClass<JArrayClass<T>, detail::JTypeArray> {
 public:
  ...
  using javaobject =
      typename JavaClass<JArrayClass<T>, detail::JTypeArray>::javaobject;
  static constexpr const char* kJavaDescriptor = nullptr;
  ...
  /// This implementation of [] returns a proxy object which then has a bunch of
  /// specializations (adopt_local free function, operator= and casting
  /// overloads on the ElementProxy) that can make code look like it is dealing
  /// with a T rather than an obvious proxy. In particular, the proxy in this
  /// iteration does not read a value and therefore does not create a LocalRef
  /// until one of these other operators is used. There are certainly holes that
  /// you may find by using idioms that haven't been tried yet. Consider
...
template <typename T>
local_ref<typename JArrayClass<T>::javaobject> adopt_local_array(
    jobjectArray ref);

template <typename Target>
local_ref<typename Target::javaentry> adopt_local(
    detail::ElementProxy<Target> elementProxy) {
...
/// This is an empty holder by itself. Construct a PinnedPrimitiveArray to
/// actually interact with the elements of the array.
template <typename JArrayType>
class JPrimitiveArray : public JavaClass<
                            JPrimitiveArray<JArrayType>,
                            detail::JArray,
                            JArrayType> {
 ...
 public:
  ...
  /// other) or a copy of the array (in which cases changes to the view will
  /// take affect when destroyed or on calls to release()/commit()).
  ...
  PinnedPrimitiveArray<T, PinnedRegionAlloc<T>> pinRegion(
      jsize start,
      jsize length);
...
/// This currently only supports read/write access to existing java arrays. You
/// can't create a primitive array this way yet. This class also pins the entire
/// array into memory during the lifetime of the PinnedPrimitiveArray. If you
/// need to unpin the array manually, call the release() or abort() functions.
/// During a long-running block of code, you should unpin the array as soon as
/// you're done with it, to avoid holding up the Java garbage collector.
template <typename T, typename PinAlloc>
class PinnedPrimitiveArray {
 public:
  static_assert(
  ...
  void abort();
  /// If the array is a copy, copies pending changes to the underlying java
  /// array.
  void commit();
...
PlainJniRefMap(JArrayBoolean, jbooleanArray) PlainJniRefMap(
    JArrayByte,
    jbyteArray) PlainJniRefMap(JArrayChar, jcharArray)
