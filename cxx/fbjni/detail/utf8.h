...
// JNI represents strings encoded with modified version of UTF-8.  The
// difference between UTF-8 and Modified UTF-8 is that the latter support only
// 1-byte, 2-byte, and 3-byte formats. Supplementary character (4 bytes in
// unicode) needs to be represented in the form of surrogate pairs. To create a
// Modified UTF-8 surrogate pair that Dalvik would understand we take 4-byte
// unicode character, encode it with UTF-16 which gives us two 2 byte chars
// (surrogate pair) and then we encode each pair as UTF-8. This result in 2 x 3
// byte characters.  To convert modified UTF-8 to standard UTF-8, this mus tbe
// reversed.
//
// The second difference is that Modified UTF-8 is encoding NUL byte in 2-byte
// format.
//
// In order to avoid complex error handling, only a minimum of validity checking
// is done to avoid crashing.  If the input is invalid, the output may be
// invalid as well.
//
// Relevant links:
//  -
