...
TEST(SimpleFixedString_test, Tests) {
  ...
  static_assert(
      constexpr_strcmp(str.c_str(), "hello") == 0, "bad fixedstring contents!");

  static constexpr auto substr = str.substr(1, str.size() - 2);
  static_assert(substr.size() == 3, "substr has wrong size");
  static_assert(
      constexpr_strcmp(substr.c_str(), "ell") == 0, "substr is broken!");

  static constexpr auto concatLeft = "Why " + str;
  static_assert(concatLeft.size() == 9, "wrong length for Why hello");
  static_assert(
      constexpr_strcmp(concatLeft.c_str(), "Why hello") == 0,
      "left concat is broken!");

  static constexpr auto concatRight = str + " there";
  static_assert(concatRight.size() == 11, "wrong length for hello there");
  static_assert(
      constexpr_strcmp(concatRight.c_str(), "hello there") == 0,
      "right concat is broken!");

  static constexpr auto bigConcat = makeSimpleFixedString("Let's ") +
      makeSimpleFixedString("make ") + makeSimpleFixedString("a ") +
      makeSimpleFixedString("long ") + makeSimpleFixedString("string!");
  static_assert(bigConcat.size() == 25, "big concat has wrong size");
  static_assert(
      constexpr_strcmp(bigConcat.c_str(), "Let's make a long string!") == 0,
      "bigConcat is broken!");
}
